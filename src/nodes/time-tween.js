  ...
  Iframework.NativeNodes['time-tween'] = Iframework.NativeNodes['time'].extend({
    ...
    setupTween: function () {
      ...
      if (window.TWEEN) {
        ...
        this.$('.tween-view').attr({
          src: 'libs/Tween/' + this._type + '.' + ease + '.png',
        });
    ...
    gotoPercent: function (p) {
      if (this.tween && this.tween.easing) {
        ...
        this.tween.value =
          this._from + (this._to - this._from) * this.tween.easing(p);
    ...
    redraw: function () {
      if (this.tween) {
        this.$('.tween-value').text(
          Math.round(this.tween.value * 100000000) / 100000000
        );
        this.$('.tween-progress').css(
          'left',
          this.tween.percent * 96 + 2 + '%'
        );
      }
    },
    ...
    inputs: {
      ...
      type: {
        type: 'string',
        description:
          'Linear Quadratic Cubic Quartic Quintic Sinusoidal Exponential Circular Elastic Back Bounce. For a visual explanation see: http://sole.github.com/tween.js/examples/03_graphs.html',
        options:
          'Linear Quadratic Cubic Quartic Quintic Sinusoidal Exponential Circular Elastic Back Bounce'.split(
            ' '
          ),
        default: 'Sinusoidal',
      },
      ease: {
        type: 'string',
        description:
          'In, Out, or InOut for all except Linear, which is None. For a visual explanation see: http://sole.github.com/tween.js/examples/03_graphs.html',
