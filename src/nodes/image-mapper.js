...
$(function () {
  ...
  var quadWarpMatrix = function (w, h, xTL, yTL, xTR, yTR, xBL, yBL, xBR, yBR) {
    ...
    return prefixTransform(
      matrix3d(w, h, xTL, yTL, xTR, yTR, xBL, yBL, xBR, yBR)
    );
  };
  ...
  Iframework.NativeNodes['image-mapper'] = Iframework.NativeNodes[
    'image'
  ].extend({
    info: {
      title: 'mapper',
    ...
    initializeModule: function () {
      ...
      $(this.canvas).draggable('destroy').remove();
    ...
    updateControlPoints: function () {
      if (this.selected) {
        ...
        this.$('.control-point.tl').css({
          left: layer.x0 + 'px',
          top: layer.y0 + 'px',
        });
        this.$('.control-point.tr').css({
          left: layer.x1 + 'px',
          top: layer.y1 + 'px',
        });
        this.$('.control-point.bl').css({
          left: layer.x2 + 'px',
          top: layer.y2 + 'px',
        });
        this.$('.control-point.br').css({
          left: layer.x3 + 'px',
          top: layer.y3 + 'px',
        });
      }
