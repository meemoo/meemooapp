  ...
  Iframework.NativeNodes['view-layers'] = Iframework.NativeNodes['view'].extend(
    {
      info: {
        title: 'layers',
        description:
          'can get any of the canvases in the graph and make a stack of them',
      },
        ...
        _.each(
          ...
          function (canvas) {
            ...
            if (canvas.className !== 'layers-copy') {
              var li = $(
                '<li class="ui-state-default" style="padding:5px;" />'
              ).text(canvas.id);
              var id = canvas.id.split('-')[1];
              var vis = $(
                '<input type="checkbox" id="vis-' +
                  canvas.id +
                  '" class="vis" title="visible" />'
              ).data({
                canvas: canvas,
                id: id,
              });
      ...
      setVis: function (event) {
        ...
        if (event.target.checked) {
          ...
          if (!this.visible[id]) {
            var vis = {};
            this.visible[id] = vis;
            vis.nativeView = this.model.parentGraph
              .get('nodes')
              .get(id).view.Native;
