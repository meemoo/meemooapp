...
$(function () {
  ...
  */
  ...
  var template =
    '<pre class="output" style="font-family:monospace; font-size:12px; line-height:70%;"></pre>' +
    '<div class="char-sets">' +
  ...
  Iframework.NativeNodes['image-ascii'] = Iframework.NativeNodes[
    'image'
  ].extend({
    info: {
      title: 'ascii',
    ...
    redraw: function () {
      ...
      if (this._image) {
        ...
        this.outputString =
          (this._color ? '<pre>' : '') + string + (this._color ? '</pre>' : '');
        this.inputsend();
      }
    },
    inputs: {
      ...
      characters: {
        type: 'string',
        description:
          'the characters that will be used in the conversion, dark to light',
        default: '#MBX$PxOo=*!~-^,._ ',
      },
      color: {
        type: 'boolean',
