...
$(function () {
  ...
  var rgbToHex = function (rgb) {
    ...
    if ((result = rgbRegex.exec(rgb))) {
      hex =
        '#' +
        componentToHex(result[1]) +
        componentToHex(result[2]) +
        componentToHex(result[3]);
      return hex;
    }
    return false;
  };
  ...
  Iframework.NativeNodes['image-palette'] = Iframework.NativeNodes[
    'image'
  ].extend({
    ...
    inputimage: function (image) {
      ...
      if (
        image.width > 2 &&
        image.height > 2 &&
        image.width * image.height > 500 * 500
      ) {
    ...
    inputrandom: function (index) {
      if (this._palette && this._palette.length > 0) {
        var color =
          this._palette[Math.floor(this._palette.length * Math.random())];
