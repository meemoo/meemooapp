...
$(function () {
  var template =
    '<div class="info"><span class="equation"></span> <span class="solution"></span></div>';
  ...
  Iframework.NativeNodes['util-math'] = Iframework.NativeNodes['util'].extend({
    ...
    inputs: {
      ...
      equation: {
        type: 'string',
        description:
          'f(x,y,z) (function calls), ^ (exponentiation), *, /, and % (multiplication, division, and remainder), and finally +, -, and || (addition, subtraction, and string concatenation) ... sin(x), cos(x), tan(x), asin(x), acos(x), atan(x), sqrt(x), log(x), abs(x), ceil(x), floor(x), round(x), exp(x) ... random(n), fac(n), min(a,b,…), max(a,b,…), pyt(a, b), pow(x, y), atan2(y, x)',
        default: 'x',
      },
      evaluate: {
        type: 'bang',
