...
$(function () {
  Iframework.NativeNodes['time-throttle'] = Iframework.NativeNodes[
    'time'
  ].extend({
    info: {
      title: 'throttle',
      description:
        'too many fps? use throttle to limit data rate passing through flow.',
    },
    ...
    inputfps: function (fps) {
      this._ms = 1000 / fps;
      this.$('.info').text(
        fps + 'fps, ' + Math.round(this._ms * 100) / 100 + 'ms'
      );
    },
    _lastTime: 0,
