...
$(function () {
  ...
  Iframework.NativeNodes['image-particles'] = Iframework.NativeNodes[
    'image'
  ].extend({
    info: {
      title: 'particles',
    ...
    redraw: function (timestamp) {
      ...
      if (this._lastTime > 0) {
        ...
        this._spawnNext += Math.min(
          this._spawnRate / 10,
          this._spawnRate * ((timestamp - this._lastTime) / 1000)
        );
      }
      ...
      // Spawn new particles
      while (
        this.particles.length < this._maxParticles &&
        this._spawnNext > 1
      ) {
        var angle =
          (this._angle - 0.25 + randomPlusOrMinus(this._angleSpread)) *
          2 *
          Math.PI;
      ...
      for (var i = 0; i < this.particles.length; i++) {
        ...
        if (this._matchAngle) {
          this.context.rotate(particle.angle);
        }
        if (draw) {
    ...
    inputs: {
      ...
      life: {
        type: 'float',
        description:
          'particle lifetime in seconds (0 will live until off edge)',
        default: 1,
      },
      loop: {
        type: 'boolean',
